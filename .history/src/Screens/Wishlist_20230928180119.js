...
import { addToCart, getWishlist, removeWishlist } from "../Repository/Api";
...
const Wishlist = () => {
  ...
  const MoveToCart = (i , product, sizeId, ) => {
          ...
          <div className="wishlist">
            {product?.map((i, index) => (
              <div className="Main" key={index}>
                ...
                <button onClick={() => MoveToCart(i , i._id , i.sizePrice?.[0]?._id)}>Move to bag</button>
              </div>
            ))}
          </div>
        </>
